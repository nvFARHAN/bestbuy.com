...
let search = async (x, y, z, sort) => {
  imagg.src =
    "https://media3.giphy.com/media/wvtt4mtViPOSrLYNFh/200w.webp?cid=ecf05e47zr7vbmkescdla1njjrmxplpwqaerup349a5ol3pj&rid=200w.webp&ct=g";
  attach.innerHTML = "";
  ...
  try {
    if ((y == undefined) & (z == undefined)) {
      rep = await fetch(
        `https://api.bestbuy.com/v1/products((search=${x}))?apiKey=pMSIDfTbv0JiUHOeHleFgKUF&pageSize=20&sort=${sort}&page=1&format=json`
      );
    } else if (z == undefined) {
      rep = await fetch(
        `https://api.bestbuy.com/v1/products((search=${x}&search=${y}))?apiKey=pMSIDfTbv0JiUHOeHleFgKUF&pageSize=20&sort=${sort}&page=1&format=json`
      );
    } else {
      rep = await fetch(
        `https://api.bestbuy.com/v1/products((search=${x}&search=${y}&search=${z}))?apiKey=pMSIDfTbv0JiUHOeHleFgKUF&pageSize=20&sort=${sort}&page=1&format=json`
      );
    }
...
const append = (data) => {
  ...
  if (data.length == 0) {
    imagg.src =
      "https://media4.giphy.com/media/8L0Pky6C83SzkzU55a/200w.webp?cid=ecf05e47ag1e9g8kjrxojnufeato3c33ghg4r2bcjb22bj7q&rid=200w.webp&ct=g";
    attach.append(imagg);
  }
